...
LEVEL=0
if [ -f wallet.dat -a -f addr.dat -a -f blCPLdex.dat -a -f blk0001.dat ]; then LEVEL=1; fi
if [ -f wallet.dat -a -f peers.dat -a -f blCPLdex.dat -a -f blk0001.dat ]; then LEVEL=2; fi
...
if [ $LEVEL -ge 3 ]; then FILES=$(echo $FILES blk????.dat blCPLdex.dat); fi
