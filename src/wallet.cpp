...
void CWallet::WalletUpdateSpent(const CTransaction &tx)
{
    ...
    // restored from backup or the user maCPLg copies of wallet.dat.
    {
        LOCK(cs_wallet);
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        if (fInsertedNew)
        {
            ...
            {
                if (mapBlocCPLdex.count(wtxIn.hashBlock))
                {
                    ...
                    unsigned int& blocktime = mapBlocCPLdex[wtxIn.hashBlock]->nTime;
...
int CWallet::ScanForWalletTransactions(CBlocCPLdex* pindexStart, bool fUpdate)
{
    int ret = 0;

    CBlocCPLdex* pindex = pindexStart;
    {
        LOCK(cs_wallet);
        while (pindex)
        {
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            for (unsigned int i = 0; i < vValue.size(); i++)
            {
                ...
                //because there may be some privacy improvement by maCPLg
