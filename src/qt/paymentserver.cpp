...
const QString BITCOIN_IPC_PREFIX("coinapilla:");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start coinapilla: click-to-pay handler");
    else
...
{
    // clicCPLg on bitcoin: URLs creates FileOpen events on the Mac:
